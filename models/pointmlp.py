...
# 创建 LSTM 模型
class LSTM_clip(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers):
        super(LSTM_clip, self).__init__()
    ...
    def forward(self, x):
        # 初始化隐藏状态和细胞状态
        h_0 = torch.zeros(self.num_layers, x.size(0), self.lstm.hidden_size).to("cuda")
        c_0 = torch.zeros(self.num_layers, x.size(0), self.lstm.hidden_size).to("cuda")
        
        self.lstm.flatten_parameters()
        # 前向传播 LSTM
        out, _ = self.lstm(x, (h_0, c_0))
        
        # 将 LSTM 输出的最后一个时间步的隐藏状态展平
...
class LocalGrouper(nn.Module):
    ...
    def point_resample(self,points,npoints):
    ...
    def forward(self, xyz, points):
        ...
        new_points = torch.cat([grouped_points, new_points.view(B, S, 1, -1).repeat(1, 1, self.kneighbors, 1)], dim=-1) # cat中心点信息和邻居信息
...
class Model(nn.Module):
    def __init__(self, points=5000, class_num=120, embed_dim=64, groups=1, res_expansion=1.0,sample="fast",
        ...
        self.classifier_mid = nn.Sequential(
    ...
    def trans_2_pc(self, x, n, c):
        ...
        xyz = x.permute(0, 2, 1).float()       # -> [B, 16000, c+2]
...
def pointMLP(num_classes=60, num_points=4096, sample="fast",**kwargs) -> Model:
    return Model(points=num_points, class_num=num_classes, embed_dim=64, groups=1, res_expansion=1.0, 
                   sample=sample,  # points=1024--*5-->5120 2的倍数 4096
                   activation="relu", bias=False, use_xyz=False, normalize="anchor",
                   dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                   k_neighbors=[24, 24, 24, 24], reducers=[2, 2, 2, 2], **kwargs)

def pointMLP_5(num_classes=60, num_points=4096,**kwargs) -> Model:    # 5 layers
