...
import numpy as np
import sys
sys.path.append("/data/ljy/Skeleton2Point/")
import graph
...
class ConvBNReLURes1D_inception(nn.Module):
    def __init__(self, channel, kernel_size=1, groups=1, res_expansion=1.0, bias=True, activation='relu'):
        ...
        # self.inception1 =  Inception(in_c=channel, c1=channel_res//4, c2=(channel_res//2, channel_res//4), c3=(channel_res//2, channel_res//4), c4=channel_res//4)
        # self.inception2 = Inception(in_c=channel_res, c1=channel//4, c2=(channel//2, channel//4), c3=(channel//2, channel//4), c4=channel//4)
        ...
        self.net1 = nn.Sequential(
            nn.Conv1d(in_channels=channel, out_channels=channel_res,
                      kernel_size=kernel_size, groups=groups, bias=bias),
            # self.inception1,
        ...
        self.net2 = nn.Sequential(
            nn.Conv1d(in_channels=channel_res, out_channels=channel,
                        kernel_size=kernel_size, bias=bias),
            # self.inception2,
            nn.BatchNorm1d(channel)
        )
...
class PosExtraction(nn.Module):
    def __init__(self, channels, blocks=1, groups=1, res_expansion=1, bias=True, activation='relu'):
        ...
        for _ in range(blocks):
            ...
            )   # ConvBNReLURes1D ConvBNReLURes1D_inception
        self.operation = nn.Sequential(*operation)
...
class Model(nn.Module):
    def __init__(self, points=5000, class_num=120, embed_dim=64, groups=1, res_expansion=1.0,
                 ...
                 graph = 'graph.ntu_rgb_d.Graph', k=1,
                 **kwargs):
        super(Model, self).__init__()
        self.stages = len(pre_blocks)
        self.class_num = class_num
        self.points = points
        self.A_vector = self.get_A(graph, k)    # ljy revised
        self.embedding = ConvBNReLU1D(5, embed_dim, bias=bias, activation=activation)#0614 c=3->c=5
        self.pos_embedding = nn.Parameter(torch.randn(1, embed_dim, 3200)) # ljy revised;num_point->3200
    ...
    def get_A(self, graph, k):  # ljy revised
        Graph = import_class(graph)()
        A_outward = Graph.A_outward_binary
        I = np.eye(25)   # num_point
        if k == 0:
            return torch.tensor(I, dtype=torch.float32)
        # return  torch.tensor(I - np.linalg.matrix_power(A_outward, k), dtype=torch.float32)   # 原工作
        return  torch.tensor(I + np.linalg.matrix_power(A_outward, k), dtype=torch.float32)     # 相当于残差
    ...
    # def forward_embeddings(self, x):
    #     b, c, img_v, img_t, m = x.size()  # [15, 3, 64, 25, 2]
    #     range_v = torch.arange(0, img_v, step=1)/ (img_v - 1.0) # torch.Size([64])
    #     range_t = torch.arange(0, img_t, step=1)/ (img_t - 1.0) # torch.Size([25])
    #     fea_pos = torch.stack(torch.meshgrid(range_v, range_t, indexing='ij'), dim=-1).float() # torch.Size([64, 25, 2])
    #     fea_pos = fea_pos
    #     fea_pos = fea_pos - 0.5
    #     pos = fea_pos.permute(2, 0, 1).unsqueeze(dim=0).expand(b*m, -1, -1, -1)#[8, 2, 64, 25]
        
    #     # [b, c, T, V, M] -> [b, M, c, T, V] -> [b*M, c, T, V]
    #     x = x.permute(0, 4, 1, 2, 3).reshape(b*m, c, img_v, img_t)
    #     x = torch.cat([x[torch.arange(b*m),:,:,:], pos], dim=1)#for循环优化
    #     x = x.reshape(b, m, c+2, img_v, img_t).permute(0,2,3,4,1) # [16, 5, 64, 25] -> [8, 2, 5, 64, 25] -> [8, 5, 64, 25, 2]
    #     return x

    def forward(self, x): # 原始xyz乘A
        x = self.forward_embeddings(x) # -> [B, C+2, T, V, M]
        n, c, T, V, M = x.size()
        # ljy revised--------------
        x = rearrange(x, 'n c T V M -> (n M T) V c').contiguous()
        x = self.A_vector.to(x.device).expand(n*M*T, -1, -1) @ x   # (n M T) V c
        x = rearrange(x, '(n M T) V c -> n c (M V T)',M=M,T=T,n=n).contiguous()
        xyz = x.permute(0, 2, 1)       # -> [B, 16000, c+2]
        x=x.float()
        x = self.embedding(x)          # -> [B, 64, 16000]
        x = x+ self.pos_embedding      # [B, C=64, V=16000]
        # x = self.data_bn(x)
    ...
    # def forward(self, x): # embedding then A
    #     x = self.forward_embeddings(x) # -> [B, C+2, T, V, M]
    #     n, c, T, V, M = x.size()
    #     x = x.reshape(n, c, -1) 
    #     xyz = x.permute(0, 2, 1)       # -> [B, 16000, c+2]
    ...
    #     # ljy revised--------------在embedding后做A乘法
    #     x = rearrange(x, 'n c (T V M) -> (n M T) V c',M=M,T=T,V=V).contiguous()
    #     x = self.A_vector.to(x.device).expand(n*M*T, -1, -1) @ x    # (n M T) V c .to(torch.double)
    #     x = rearrange(x, '(n M T) V c -> n c (M V T)',M=M,T=T,n=n).contiguous()
    #     x = x.float()
    #     x = x+ self.pos_embedding      # [B, C=64, V=16000]
